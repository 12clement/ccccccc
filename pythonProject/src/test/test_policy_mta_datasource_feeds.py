...
def assert_dataframe_equal(res_flatten_kafka_source_df, res_flatten_s3_df,  res_flatten_rds_df):
    ...
    out_flatten_kafka_source_df = res_flatten_kafka_source_df.groupBy(
        'payload_policyDetails_features_description',
        'payload_policyDetails_propensityScores_scoreType_description').count()
    outList = out_flatten_kafka_source_df.collect()
    feat_desc_lst = []
    for i in outList:
        feat_desc_lst.append((i['payload_policyDetails_features_description'],
                             i['payload_policyDetails_propensityScores_scoreType_description']))

    for feat_desc, searchType_desc in feat_desc_lst:

        mini_flatten_kafka_source_df = res_flatten_kafka_source_df.filter(
            (res_flatten_kafka_source_df['payload_policyDetails_features_description'] == feat_desc) &
            (res_flatten_kafka_source_df['payload_policyDetails_propensityScores_scoreType_description'] \
             == searchType_desc))
        mini_flatten_s3_df = res_flatten_s3_df.filter(
            (res_flatten_s3_df['payload_policyDetails_features_description'] == feat_desc) &
            (res_flatten_s3_df[
                 'payload_policyDetails_propensityScores_scoreType_description'] == searchType_desc))
        ...
        print(f"Kafka data successfully matches s3 data for fields based on '{feat_desc}' and '{searchType_desc}'")

        '''
         Due to large volume size comparison above should be completed and commented before running below
        '''
        #
        ...
        # print(f"Kafka data successfully matches rds data for fields based on '{feat_desc}' and '{searchType_desc}'")

def load_data(spark):
    dataframes = []

    df = spark.read.option("multiline", "true") \
        .json("C:\\Users\\ctagb\\workspace_python\\pythonProject\\pythonProject\\data\\policy_mta_kafka.json")
    res_flatten_kafka_source_df = flatten_and_sort_dataframe(df)
    dataframes.append(res_flatten_kafka_source_df)
    print("========================================================")

    df_s3 = spark.read \
        .text("C:\\Users\\ctagb\\workspace_python\\pythonProject\\pythonProject\\data\\policy_mta_s3.txt")
    ...
    df_rds = spark.read.option("multiline", "true") \
        .json("C:\\Users\\ctagb\\workspace_python\\pythonProject\\pythonProject\\data\\policy_mta_rds.txt")
