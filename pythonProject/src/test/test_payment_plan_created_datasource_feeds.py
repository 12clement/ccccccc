...
from src.utils.definitions import payment_plan_mandatory_column_value_list
...
def validate_not_null_fields(res_flatten_kafka_source_df):
    ...
    for field in payment_plan_mandatory_column_value_list:
...
def assert_dataframe_equal(res_flatten_kafka_source_df,  res_flatten_rds_df):
...
def load_data(spark):
    ...
    df = spark.read.option("multiline", "true") \
        .json("C:\\Users\\ctagb\\workspace_python\\pythonProject\\pythonProject\\data\\payment_plan_created_kafka.json")
    ...
    pd_flatten_kafka_source_df = res_flatten_kafka_source_df.toPandas()
    pd_flatten_kafka_source_df.dropna(inplace=True, axis='columns')
    res_flatten_kafka_source_df = spark.createDataFrame(pd_flatten_kafka_source_df)
    # res_flatten_kafka_source_df.printSchema()
    dataframes.append(res_flatten_kafka_source_df)


    df_rds = spark.read.option("multiline", "true") \
        .json("C:\\Users\\ctagb\\workspace_python\\pythonProject\\pythonProject\\data\\payment_plan_created_rds.txt")
    ...
    pd_flatten_rds_df = res_flatten_rds_df.toPandas()
    pd_flatten_rds_df.dropna(inplace=True, axis='columns')
    res_flatten_rds_df = spark.createDataFrame(pd_flatten_kafka_source_df)
...
if __name__ == "__main__":
    ...
    assert_dataframe_equal(dfList[0], dfList[1])
