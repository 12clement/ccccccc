...
from src.utils.definitions import payment_admin_pymnt_credit_changed_mandatory_column_value_list
...
def test_validate_not_null_fields(res_flatten_kafka_source_df):
    pass
     # validate nullable false value kafka_source,s3 and rds
    for field in payment_admin_pymnt_credit_changed_mandatory_column_value_list:
...
def test_compare_dataframe_schemas(dfs):
    """
...
def test_assert_dataframe_equal_small_payload(res_flatten_kafka_source_df,  res_flatten_rds_df):
...
def load_data(spark):
    ...
    df = spark.read.option("multiline", "true") \
        .json("C:\\Users\\ctagb\\workspace_python\\pythonProject\\pythonProject\\data\\pymnt_adm_payment_credit_changed_kafka.json")
...
if __name__ == "__main__":
    ...
    test_compare_dataframe_schemas(dfList)

    # Call method to check for nullability of mandatory fields.
    test_validate_not_null_fields(dfList[0])

    # Call method to compare dataframes values
    test_assert_dataframe_equal_small_payload(dfList[0], dfList[1])
